...
// import addFavorit from '../../../services/marketPlace';
// import { saveMarketplace } from '../../../store/Reducers/marketplace';
// import { getMarketplace } from '../../../services/Home';
import { saveMarketplace } from '@/store/reducers/marketplace';
import { getMarketplace } from '@/utils/Home';
import { CommonButton } from '@/components/common/loader/Loader';
const ExpandComponent = ({ key, faq, seeMore, less, seeLess, setIsExpanded, isExpanded, togglePopup, handleOpenLink, handleStop }) => {
  const capitalize = (s) => {
    ...
    <>
        ...
        <div className="w-full flex flex-col items-center justify-center flex-wrap lg:flex-nowrap rounded-lg h-full">
          ...
          <div className="w-full flex justify-center gap-x-1 items-center">
            <div className={styles.socialIcon}>
              <div className="flex gap-x-1 text-[15px] items-center">
                {faq?.priceRangeMin}
                {/* <img src={BnbImage} alt="bnb" className="w-4 h-4 object-contain" /> */}
              </div> -
              <div className="flex gap-x-1 text-[15px] items-center">
                {faq?.priceRangeMax}{" "}
                {/* <img src={BnbImage} alt="bnb" className="w-4 h-4 object-contain" /> */}
            ...
            <div onClick={(e) => handleStop(e)}>
              <div className="flex items-center justify-end mt-0">
                <Button
                  primary={faq?.votes > 0 ? true : false}
                  onClick={() => togglePopup(faq)}
                >
                  {faq?.votes}
                </Button>
              </div>
      ...
      <div className="w-full hidden lg:block">
        <div className="w-full flex flex-wrap lg:flex-nowrap rounded-lg h-full">
          <div className="w-full lg:w-[22%] max-h-[400px] rounded-xl  p-0 lg:p-2">
            <img
              src={faq?.coverImg}
          ...
          <div className="w-full flex flex-col justify-between lg:w-[78%]" onClick={() => setIsExpanded(!isExpanded)}>
            <div>
              <div className="mt-2 lg:mt-0 flex items-center flex-wrap justify-between">
                ...
                <div onClick={(e) => handleStop(e)}>
                  <div className="flex items-center justify-end mt-0 gap-x-1">
                    {faveIndex === key && favIconLoading ?
                      <CommonButton
                        loading={favIconLoading}
                        width="20px"
              ...
              {faq?.description?.length > 250 ? (
                less ? (
                  <div className="text-[16px]  font-normal dark:text-white text-black mt-5">
                    {faq?.description.substring(0, 250)}...
                    <span
                      className="underline ml-1 text-blue-500"
                      onClick={seeMore}
